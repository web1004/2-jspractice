(function(){
  ...
  //(5)
  /* 웹페이지에 접속했을때 localStorage에 quote라는 키값의 데이터가 존재하고 createDate값이 오늘의 날짜와 동일하다면 getQuote함수를 호출하지 않고 그렇지 않을경우만 getQuote함수를 호출한다.
  하루에 한개의 명언만 볼수 있고 날짜가 변경될때마다 다른 명언을 볼수 있게 하기 위해 명언을 로컬스토리지에 저장해서 동일한 날짜라면 명언API를 호출하지 않고 로컬스토리지에 저장된 명언을 가져올수 있게 한다.*/
